...
TRANS_SUBS_FOR_AUDIO_FILE = os.path.join(_paths["temp"], "audio", "trans_subs_for_audio.srt")
SRC_SUBS_FOR_AUDIO_FILE = os.path.join(_paths["temp"], "audio", "src_subs_for_audio.srt")
ESTIMATOR = None
...
def check_len_then_trim(text, duration):
    ...
    if estimated_duration > duration:
        ...
        try:
            ...
            rprint(
                "[bold red]🚫 AI refused to answer due to sensitivity, so manually remove punctuation[/bold red]"
            )
...
def process_srt():
    ...
    for block in content.strip().split("\n\n"):
        ...
        try:
            ...
            rprint(
                Panel(
                    f"Unable to parse subtitle block '{block}', error: {str(e)}, skipping this subtitle block.",
                    title="Error",
                    border_style="red",
                )
            )
    ...
    while i < len(df):
        ...
        if df.loc[i, "duration"] < MIN_SUB_DUR:
            if (
                i < len(df) - 1
                and time_diff_seconds(df.loc[i, "start_time"], df.loc[i + 1, "start_time"], today)
                < MIN_SUB_DUR
            ):
                ...
                df.loc[i, "duration"] = time_diff_seconds(
                    df.loc[i, "start_time"], df.loc[i, "end_time"], today
                )
                df = df.drop(i + 1).reset_index(drop=True)
            else:
                if i < len(df) - 1:  # Not the last audio
                    rprint(
                        f"[bold blue]Extending subtitle {i+1} duration to {MIN_SUB_DUR} seconds[/bold blue]"
                    )
                    df.loc[i, "end_time"] = (
                    ...
                    rprint(
                        f"[bold red]The last subtitle {i+1} duration is less than {MIN_SUB_DUR} seconds, but not extending[/bold red]"
                    )
                i += 1
        else:
            i += 1
...
def gen_audio_task_main():
    ...
    rprint(
        Panel(f"Successfully generated {_8_1_AUDIO_TASK}", title="Success", border_style="green")
    )
