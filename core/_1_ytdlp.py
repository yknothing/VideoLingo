...
from core.utils.security_utils import sanitize_filename, sanitize_path, validate_proxy_url
...
def find_best_video_file(save_path, allowed_formats):
    ...
    for file_path in glob.glob(os.path.join(save_path, "*")):
        ...
        is_valid, error_msg = validate_video_file(file_path)
        if not is_valid:
            rprint(f"[yellow]Skipping invalid file {os.path.basename(file_path)}: {error_msg}[/yellow]")
            continue
...
def intelligent_retry_download(download_func, max_retries=3, initial_wait=5):
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            rprint(f"[yellow]Download attempt {attempt + 1} failed ({category}). Retrying in {wait_time}s...[/yellow]")
...
def download_video_ytdlp(url, save_path=None, resolution=None, progress_callback=None):
    ...
    print(f"[DEBUG] download_video_ytdlp called with resolution: '{resolution}' (type: {type(resolution)})")
    ...
    def attempt_download():
        try:
            return download_via_command(url, save_path, resolution, progress_callback, outtmpl=None)
        except Exception as e:
            rprint(f"[yellow]External yt-dlp failed: {e}. Trying Python API...[/yellow]")
            # Fallback to Python API
            return download_via_python_api(url, save_path, resolution, progress_callback, outtmpl=None)
    ...
    if not downloaded_file or not os.path.exists(downloaded_file):
        rprint(f"[yellow]Download function returned invalid path, searching for downloaded file...[/yellow]")
        ...
        try:
            ...
            raise Exception(f"Download failed: Unable to locate downloaded file. Original error: {fallback_error}")
    ...
    rprint(f"[green]Download completed successfully: {os.path.basename(downloaded_file)} ({file_size:.1f}MB)[/green]")
    
    return downloaded_file

def download_via_python_api(url, save_path, resolution, progress_callback=None, outtmpl=None):
    """
    ...
    if "proxy" not in ydl_opts:
        ...
        if proxy_env and validate_proxy_url(proxy_env):
            ...
            rprint(f"[yellow]Warning: Invalid proxy URL format in environment: {proxy_env}[/yellow]")
    ...
    if downloaded_holder['path'] and os.path.exists(downloaded_holder['path']):
        ...
        rprint("[yellow]Could not determine file path from Python API, searching directory...[/yellow]")
...
def download_via_command(url, save_path=None, resolution='1080', progress_callback=None, outtmpl=None):
...
def find_video_files(save_path=None):
    ...
    video_files = [file for file in video_files if not any(file.startswith(prefix) for prefix in ["output/output", "/output"])]
    ...
    rprint(f"[yellow]Multiple video files found ({len(video_files)}). Selecting the best one...[/yellow]")
    ...
    if best_file is None:
        ...
        for file_path in video_files:
            ...
            is_valid, error_msg = validate_video_file(file_path)
            rprint(f"  - {os.path.basename(file_path)}: {size_mb:.1f}MB, Valid: {is_valid}, Error: {error_msg}")
        raise ValueError("No valid video files found after validation. Please check the download and try again.")
    
    # Preserve all files - no cleanup per user requirements
    
    rprint(f"[green]Selected video file: {os.path.basename(best_file)} ({os.path.getsize(best_file)/(1024*1024):.1f}MB)[/green]")
    return best_file
