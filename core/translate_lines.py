...
def translate_lines(
    ...
    def retry_translation(prompt, length, step_name):
        def valid_faith(response_data):
            return valid_translate_result(
                response_data, [str(i) for i in range(1, length + 1)], ["direct"]
            )

        def valid_express(response_data):
            return valid_translate_result(
                response_data, [str(i) for i in range(1, length + 1)], ["free"]
            )
        ...
        raise ValueError(
            f"[red]❌ {step_name.capitalize()} translation of block {index} failed after 3 retries. Please check `output/gpt_log/error.json` for more details.[/red]"
        )
    ...
    if not reflect_translate:
        # If reflect_translate is False or not set, use faithful translation directly
        translate_result = "\n".join(
            [faith_result[i]["direct"].strip() for i in faith_result]
        )
    ...
    translate_result = "\n".join(
        [express_result[i]["free"].replace("\n", " ").strip() for i in express_result]
    )

    if len(lines.split("\n")) != len(translate_result.split("\n")):
        console.print(
            Panel(
                f"[red]❌ Translation of block {index} failed, Length Mismatch, Please check `output/gpt_log/translate_expressiveness.json`[/red]"
            )
        )
...
def translate_lines_batch(chunks_data, theme_prompt):
    ...
    def valid_batch_faith(response_data):
        return valid_translate_result(
            response_data, [str(i) for i in range(1, len(all_lines) + 1)], ["direct"]
        )

    def valid_batch_express(response_data):
        return valid_translate_result(
            response_data, [str(i) for i in range(1, len(all_lines) + 1)], ["free"]
        )

    # Faithfulness step
    prompt1 = get_prompt_faithfulness(all_lines_text, shared_prompt)
    faith_result = ask_gpt(
        prompt1,
        resp_type="json",
        valid_def=valid_batch_faith,
        log_title="translate_batch_faithfulness",
    )
    ...
    if not reflect_translate:
        ...
        express_result = ask_gpt(
            prompt2,
            resp_type="json",
            valid_def=valid_batch_express,
            log_title="translate_batch_expressiveness",
        )
    ...
    for chunk_data in chunks_data:
        ...
        table = Table(
            title=f"Batch Translation Results - Chunk {chunk_idx}",
            show_header=False,
            box=box.ROUNDED,
        )
        table.add_column("Translations", style="bold")
        ...
        for i in range(mapping["start"], mapping["end"] + 1):
            ...
            table.add_row(
                f"[magenta]Direct:  {final_result[line_key]['direct']}[/magenta]"
            )
            if reflect_translate and "free" in final_result[line_key]:
                table.add_row(
                    f"[green]Free:    {final_result[line_key]['free']}[/green]"
                )
            if i < mapping["end"]:
