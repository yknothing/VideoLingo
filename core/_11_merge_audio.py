...
DUB_VOCAL_FILE = os.path.join(_paths["output"], "dub.mp3")
...
def load_and_flatten_data(excel_file):
    ...
    new_sub_times = [
        eval(time) if isinstance(time, str) else time for time in df["new_sub_times"].tolist()
    ]
    ...
    with Progress(
        ...
        for i, (audio_file, time_range) in enumerate(zip(audios, new_sub_times)):
            if not os.path.exists(audio_file):
                console.print(
                    f"[bold yellow]⚠️  Warning: File {audio_file} does not exist, skipping...[/bold yellow]"
                )
            ...
            if i > 0:
                ...
                if silence_duration > 0:
                    silence = AudioSegment.silent(
                        duration=int(silence_duration * 1000), frame_rate=sample_rate
                    )
                    merged_audio += silence
            elif start_time > 0:
                silence = AudioSegment.silent(
                    duration=int(start_time * 1000), frame_rate=sample_rate
                )
                merged_audio += silence
